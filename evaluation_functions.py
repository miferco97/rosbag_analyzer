...
def compareStateEstimationWithGroundTruth(se_topic, gt_topic):
    ...
    se_positions = np.array(
        [
            se_topic.getSyncData(getXpose),
            se_topic.getSyncData(getYpose),
            se_topic.getSyncData(getZpose),
        ]
    )
    gt_positions = np.array(
        [
            gt_topic.getSyncData(getXpose),
            gt_topic.getSyncData(getYpose),
            gt_topic.getSyncData(getZpose),
        ]
    )
...
def trajectoryFollowingEvaluation(traj_topic, estimation_topic):
    ...
    se_positions = np.array(
        [
            estimation_topic.getSyncData(getXpose),
    ...
    traj_positions = np.array(
        [
            traj_topic.getSyncData(lambda x: x.positions[0]),
...
def SpeedAnalysis(traj_topic, estimation_topic):
    ...
    se_speeds = np.array(
        [
            estimation_topic.getSyncData(getXspeed),
            estimation_topic.getSyncData(getYspeed),
            estimation_topic.getSyncData(getZspeed),
        ]
    )

    traj_speeds = np.array(
        [
            traj_topic.getSyncData(lambda x: x.velocities[0]),
    ...
    speeds = np.sqrt(
        np.power(run_speeds[0, :], 2)
        + np.power(run_speeds[2, :], 2)
        + np.power(run_speeds[2, :], 2)
    )
