...
namespace = "/drone1/"
wanted_topics = [ namespace+"self_localization/pose", "/mocap_node/Robot_1/pose" ,namespace+"self_localization/speed" ,namespace+"motion_reference/trajectory" ]
...
if __name__=='__main__':
    ...
    estimator_pose_array = [x for x in topics_data if 'self_localization/pose' in x.topic_name ]
    ...
    estimator_speed_array = [x for x in topics_data if 'self_localization/speed' in x.topic_name ]
    if estimator_speed_array:
        estimator_speed = estimator_speed_array[0]
    else:
        speed_analysis = False

    traj_data_array =  [x for x in topics_data if 'motion_reference/trajectory' in x.topic_name ]
    if traj_data_array:
